...
export default function EstoquePage() {
  ...
  const handleRefreshComparison = async () => {
    ...
    try {
      ...
      toast.error(err?.message || 'Erro ao atualizar comparação');
    } finally {
      setIsRefreshing(false);
    }
  };

  const handleSyncToHeadshop = (productName: string) => {
    void (async () => {
      try {
        await syncToHeadshop();
        toast.success(`Estoque enviado para HeadShop`);
      } catch (err: any) {
        toast.error(err?.message || 'Erro ao enviar estoque');
      }
    })();
  };

  const handleSyncFromHeadshop = (productName: string) => {
    void (async () => {
      try {
        await syncFromHeadshop();
        toast.success(`Estoque atualizado do HeadShop`);
      } catch (err: any) {
        toast.error(err?.message || 'Erro ao puxar estoque');
      }
    })();
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-lg bg-status-warning/10"><ArrowLeftRight className="h-6 w-6 text-status-warning" /></div>
              <div>
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <Badge variant="secondary" className="mt-1 text-xs">{CATEGORY_LABELS[product.category] ?? product.category}</Badge>
                </div>
            ...
            <TableBody>
              {stockComparison.map((item) => (
                <TableRow
                  key={item.productId}
                  className={cn(
                    item.difference > 0 && 'bg-status-success/5',
                    item.difference < 0 && 'bg-status-warning/10'
                  )}
                >
                ...
                <TableCell className="text-center">
                  <Badge
                    variant={item.difference === 0 ? 'secondary' : 'default'}
                    className={cn(
                      item.difference === 0 && 'bg-muted text-foreground',
                      item.difference > 0 && 'bg-status-success text-white',
                      item.difference < 0 && 'bg-status-warning text-foreground'
                    )}
                  >
                  ...
                  <TableCell className="text-right">
                    {item.difference !== 0 && (
                      <div className="flex gap-1 justify-end">
                        <Button variant="outline" size="sm" onClick={() => handleSyncFromHeadshop(item.productName)}>← Puxar</Button>
                        <Button variant="outline" size="sm" onClick={() => handleSyncToHeadshop(item.productName)}>Enviar →</Button>
                      </div>
                    )}
                  </TableCell>
                </TableRow>
