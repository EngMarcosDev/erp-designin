import { useState, useEffect } from "react";
import { X } from "lucide-react";
import { Button } from "./ui/button";
import { Input } from "./ui/input";
import { resolveErpApiBase } from "@/lib/apiBase";
import { useAuth } from "@/contexts/AuthContext";

const SignupPopup = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [hasShown, setHasShown] = useState(false);
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const apiBase = resolveErpApiBase(import.meta.env.VITE_API_URL);
  const { user, login, logout } = useAuth();

  useEffect(() => {
    // Check if already shown in this session
    const alreadyShown = sessionStorage.getItem("signupPopupShown");
    if (alreadyShown) {
      setHasShown(true);
      return;
    }

    const handleScroll = () => {
      if (window.scrollY > 100 && !hasShown) {
        setIsOpen(true);
        setHasShown(true);
        sessionStorage.setItem("signupPopupShown", "true");
        window.removeEventListener("scroll", handleScroll);
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [hasShown]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !password) return;

    try {
      const res = await fetch(`${apiBase}/auth/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, email: email.trim(), password }),
      });
      if (!res.ok) throw new Error("Falha ao cadastrar");

      // auto-login
      await login(email.trim(), password);
    } catch (err) {
      console.error(err);
    }

    setName("");
    setEmail("");
    setPassword("");
    setIsOpen(false);
  };

  const handleGoogleSignup = async () => {
    // Placeholder for OAuth flow. For now register with empty password and try login.
    if (!email.trim()) return;
    try {
      await fetch(`${apiBase}/auth/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name || email, email: email.trim(), password: "" }),
      });
      try {
        await login(email.trim(), "");
      } catch {
        // ignore
      }
    } catch (err) {
      console.error(err);
    }
    setName("");
    setEmail("");
    setIsOpen(false);
  };

  if (!isOpen) return null;

  return (
    <>
      {/* Overlay */}
      <div
        className="fixed inset-0 bg-black/70 z-50 backdrop-blur-sm"
        onClick={() => setIsOpen(false)}
      />

      {/* Popup */}
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md bg-card rounded-2xl z-50 shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">
        {/* Header with rasta stripe */}
        <div className="h-1.5 bg-gradient-to-r from-rasta-green via-rasta-yellow to-rasta-red" />
        
        <div className="p-6">
          <button
            onClick={() => setIsOpen(false)}
            className="absolute top-4 right-4 text-muted-foreground hover:text-foreground"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="text-center mb-6">
            <h2 className="text-2xl font-display font-bold text-foreground tracking-wider">
              Efetue seu Cadastro
            </h2>
            <p className="text-muted-foreground mt-2 text-sm">
              É realmente uma maravilha ter você conosco.
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Input
                type="text"
                placeholder="Seu nome"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full bg-muted border-border focus:border-accent mb-2"
              />
              <Input
                type="email"
                placeholder="Seu e-mail"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="w-full bg-muted border-border focus:border-accent mb-2"
              />
              <Input
                type="password"
                placeholder="Senha"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className="w-full bg-muted border-border focus:border-accent"
              />
            </div>
            <Button
              type="submit"
              className="w-full bg-rasta-green hover:bg-rasta-green/90 text-white font-semibold"
            >
              Efetue seu Cadastro
            </Button>
            {user?.email && (
              <Button
                type="button"
                variant="outline"
                onClick={logout}
                className="w-full border-border hover:bg-muted"
              >
                Sair
              </Button>
            )}
          </form>

          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-border" />
            </div>
            <div className="relative flex justify-center text-xs">
              <span className="px-2 bg-card text-muted-foreground">
                ou continue com
              </span>
            </div>
          </div>

          <Button
            type="button"
            variant="outline"
            onClick={handleGoogleSignup}
            className="w-full border-border hover:bg-muted"
          >
            <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
              <path
                fill="currentColor"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="currentColor"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="currentColor"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="currentColor"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Entrar com Google
          </Button>
        </div>
      </div>
    </>
  );
};

export default SignupPopup;
